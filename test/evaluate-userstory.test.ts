...
describe('evaluateUserStory Integration Tests', () => {
    ...
    it('should improve a simple user story', async () => {
        ...
        // Act
        const result = await evaluateUserStory({ requirement: inputUserStory, sessionId: SESSION_ID });
    ...
    it('should handle empty input', async () => {
        ...
        // Act
        await expect(evaluateUserStory({ requirement: inputUserStory, sessionId: SESSION_ID }))
            .rejects
            .toThrow();
    }).timeout(TEST_TIMEOUT);
    ...
    it('should handle very short user stories', async () => {
        ...
        expect(result.proposedNewRequirement).toMatch(/As a .* I want to .* so that .*/);
    }).timeout(TEST_TIMEOUT);
    ...
    it('should handle user stories with acceptance criteria', async () => {
        ...
        expect(result.proposedNewRequirement.toLowerCase()).toContain('acceptance criteria');
    }).timeout(TEST_TIMEOUT);
}); 